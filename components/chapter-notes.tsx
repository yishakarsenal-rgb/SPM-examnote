...
const chapterBorders: Record<number, string> = {
  1: "border-cyan-500/20",
  2: "border-blue-500/20",
  3: "border-teal-500/20",
  4: "border-violet-500/20",
  5: "border-emerald-500/20",
  6: "border-orange-500/20",
  7: "border-red-500/20",
  8: "border-cyan-400/20",
};
  ...
  return (
    <div className="flex flex-col gap-6">
        ...
        <div className="flex items-start gap-4">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              <Badge
                variant="outline"
                className={cn("text-xs border", color, border)}
              >
            ...
            <h1 className={cn("text-2xl font-bold text-balance", color)}>
              {note.title}
            </h1>
          </div>
        </div>
      ...
      {activeTab === "notes" ? (
        <Accordion
          type="multiple"
          defaultValue={note.sections.map((_, i) => `section-${i}`)}
        ...
        >
          {note.sections.map((section, i) => (
            <AccordionItem
              ...
              value={`section-${i}`}
              className={cn(
                "glass-card border rounded-xl overflow-hidden",
                border,
              )}
            >
              <AccordionTrigger
                ...
                {section.calculations && section.calculations.length > 0 && (
                  <div className="mt-4 pt-4 border-t border-slate-700/50 space-y-3">
                    ...
                    {section.calculations.map((calc, j) => (
                      ...
                      >
                        <div className="text-xs font-mono text-white-300 bg-black/60 p-2 rounded border border-black-500/20">
                          {calc.formula}
                        </div>
          ...
          {note.keyTerms.map((term, i) => (
            ...
            >
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className={cn("text-sm font-semibold mb-1", color)}>
                    {term.term}
                  </p>
...
export function AllNotesOverview({
  onSelectChapter,
}: {
    ...
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Select a chapter to begin studying
        </p>
      </div>
